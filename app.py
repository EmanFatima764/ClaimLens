    ...
    for chat in st.session_state.saved_chats:
        if chat.get("id") == chat_id:
            ...
            if isinstance(messages, list):
                ...
                logger.warning(
                    f"Chat messages not a list for chat_id {chat_id}"
                )
                st.session_state.messages = []
...
def save_current_session(user_title, messages_list):
    ...
    if not isinstance(messages_list, list):
        logger.error(
            f"messages_list must be a list, got {type(messages_list)}"
        )
        return
...
with st.sidebar:
    st.markdown(
        ...
        <div style="text-align:center; padding: 0.6rem 0 1rem 0;">
            ...
            <div style="font-size: 1.25rem; font-weight: 800; color: #f5f6fa;">
                ClaimLens
            </div>
            <div style="font-size: 0.8rem; color: #8b90a8;">
    ...
    st.markdown(
        ...
        'color:#8b90a8; margin-bottom:0.5rem;">📜 Chat History</div>',
        unsafe_allow_html=True
    )
...
st.markdown(
    ...
    <div class="hero-wrap">
        ...
        <p class="hero-sub">
            Paste a claim, record your pitch, or upload an audio file —
            I'll extract the factual claims and check them for you.
        </p>
...
for message in st.session_state.messages:
    ...
    if not isinstance(message, dict):
        logger.warning(
            f"Skipping invalid message format: {type(message)}"
        )
        continue
...
st.markdown(
    '<div class="input-card-label">Provide your pitch</div>',
    unsafe_allow_html=True
)
...
st.caption(
    "Supports MP3, WAV, M4A — up to ~25MB / a few minutes of audio."
)
...
if prompt or uploaded_file or recorded_audio:
    ...
    with st.chat_message("assistant", avatar="🛡️"):
            ...
            try:
                if audio_source:
                    ...
                    if not transcript or not isinstance(transcript, str):
                        ...
                        logger.error(
                            f"Invalid transcription result: {type(transcript)}"
                        )
                        st.stop()
                ...
                if not isinstance(claims, list):
                    ...
                    logger.error(
                        f"Claims extraction returned non-list: {type(claims)}"
                    )
                    st.stop()
                    ...
                    for idx, item in enumerate(claims):
                        ...
                        try:
                            if not isinstance(item, dict):
                                logger.warning(
                                    f"Claim {idx} is not a dict, skipping"
                                )
                                continue
                            ...
                            if (
                                ...
                                logger.warning(
                                    f"Invalid claim text at index {idx}"
                                )
                                continue

                            claim_category = item.get(
                                "category",
                                "General Fact"
                            )
                            ...
                            if not isinstance(search_res, list):
                                logger.warning(
                                    f"Search returned non-list for claim: "
                                    f"{claim_text}"
                                )
                                search_res = []
                            ...
                            if not isinstance(verdict_info, dict):
                                logger.warning(
                                    f"Verdict info is not a dict for claim: "
                                    f"{claim_text}"
                                )
                            ...
                            verdict = verdict_info.get(
                                "verdict",
                                "UNVERIFIED"
                            )

                            explanation = verdict_info.get(
                                "explanation",
                            ...
                            safe_claim_text = _escape_html(
                                claim_text
                            )
                            safe_explanation = _escape_html(
                                explanation
                            )
                            safe_sources_str = _escape_html(
                                sources_str
                            )
                            safe_category = _escape_html(
                                claim_category
                            )
                            ...
                            logger.error(
                                f"Error processing claim {idx}: {str(e)}"
                            )

                            st.warning(
                                f"Error processing one of the claims: "
                                f"{str(e)}"
                            )
                ...
                logger.error(
                    f"Unexpected error in audit logic: {e}",
                    exc_info=True
                )
        ...
        if isinstance(response_markdown, str):
            ...
            logger.error(
                f"response_markdown is not a string: "
                f"{type(response_markdown)}"
            )
